...
from comfy_api_nodes.apis.stability_api import StabilityUpscaleConservativeRequest, StabilityUpscaleCreativeRequest, StabilityAsyncResponse, StabilityResultsGetResponse, StabilityStable3_5Request, StabilityStableUltraRequest, StabilityStableUltraResponse, StabilityAspectRatio, Stability_SD3_5_Model, Stability_SD3_5_GenerationMode, get_stability_style_presets, StabilityTextToAudioRequest, StabilityAudioToAudioRequest, StabilityAudioInpaintRequest, StabilityAudioResponse
from comfy_api_nodes.util import validate_audio_duration, validate_string, audio_input_to_mp3, bytesio_to_image_tensor, tensor_to_bytesio, audio_bytes_to_audio_input, sync_op, poll_op, ApiEndpoint
import torch
import base64
...
class StabilityUpscaleConservativeNode_ISO(IO.ComfyNode):
    ...
    @classmethod
    def define_schema(cls):
        return IO.Schema(node_id='StabilityUpscaleConservativeNode_ISO', display_name='Stability AI Upscale Conservative_ISO', category='api node/image/Stability AI', description=cleandoc(cls.__doc__ or ''), inputs=[IO.Image.Input('image'), IO.String.Input('prompt', multiline=True, default='', tooltip='What you wish to see in the output image. A strong, descriptive prompt that clearly defines elements, colors, and subjects will lead to better results.'), IO.Float.Input('creativity', default=0.35, min=0.2, max=0.5, step=0.01, tooltip='Controls the likelihood of creating additional details not heavily conditioned by the init image.'), IO.Int.Input('seed', default=0, min=0, max=4294967294, step=1, display_mode=IO.NumberDisplay.number, control_after_generate=True, tooltip='The random seed used for creating the noise.'), IO.String.Input('negative_prompt', default='', tooltip='Keywords of what you do not wish to see in the output image. This is an advanced feature.', force_input=True, optional=True)], outputs=[IO.Image.Output()], hidden=[IO.Hidden.auth_token_comfy_org, IO.Hidden.api_key_comfy_org, IO.Hidden.unique_id], is_api_node=True)

    @classmethod
    async def execute(cls, image: torch.Tensor, prompt: str, creativity: float, seed: int, negative_prompt: str='') -> IO.NodeOutput:
        validate_string(prompt, strip_whitespace=False)
...
class StabilityUpscaleCreativeNode_ISO(IO.ComfyNode):
    ...
    @classmethod
    def define_schema(cls):
        return IO.Schema(node_id='StabilityUpscaleCreativeNode_ISO', display_name='Stability AI Upscale Creative_ISO', category='api node/image/Stability AI', description=cleandoc(cls.__doc__ or ''), inputs=[IO.Image.Input('image'), IO.String.Input('prompt', multiline=True, default='', tooltip='What you wish to see in the output image. A strong, descriptive prompt that clearly defines elements, colors, and subjects will lead to better results.'), IO.Float.Input('creativity', default=0.3, min=0.1, max=0.5, step=0.01, tooltip='Controls the likelihood of creating additional details not heavily conditioned by the init image.'), IO.Combo.Input('style_preset', options=get_stability_style_presets(), tooltip='Optional desired style of generated image.'), IO.Int.Input('seed', default=0, min=0, max=4294967294, step=1, display_mode=IO.NumberDisplay.number, control_after_generate=True, tooltip='The random seed used for creating the noise.'), IO.String.Input('negative_prompt', default='', tooltip='Keywords of what you do not wish to see in the output image. This is an advanced feature.', force_input=True, optional=True)], outputs=[IO.Image.Output()], hidden=[IO.Hidden.auth_token_comfy_org, IO.Hidden.api_key_comfy_org, IO.Hidden.unique_id], is_api_node=True)

    @classmethod
    async def execute(cls, image: torch.Tensor, prompt: str, creativity: float, style_preset: str, seed: int, negative_prompt: str='') -> IO.NodeOutput:
        validate_string(prompt, strip_whitespace=False)
        image_binary = tensor_to_bytesio(image, total_pixels=1024 * 1024).read()
        if not negative_prompt:
            negative_prompt = None
        ...
        response_poll = await poll_op(cls, ApiEndpoint(path=f'/proxy/stability/v2beta/results/{response_api.id}'), response_model=StabilityResultsGetResponse, poll_interval=3, status_extractor=lambda x: get_async_dummy_status(x))
...
class StabilityUpscaleFastNode_ISO(IO.ComfyNode):
    ...
    @classmethod
    def define_schema(cls):
        return IO.Schema(node_id='StabilityUpscaleFastNode_ISO', display_name='Stability AI Upscale Fast_ISO', category='api node/image/Stability AI', description=cleandoc(cls.__doc__ or ''), inputs=[IO.Image.Input('image')], outputs=[IO.Image.Output()], hidden=[IO.Hidden.auth_token_comfy_org, IO.Hidden.api_key_comfy_org, IO.Hidden.unique_id], is_api_node=True)

    @classmethod
    async def execute(cls, image: torch.Tensor) -> IO.NodeOutput:
        image_binary = tensor_to_bytesio(image, total_pixels=4096 * 4096).read()
        files = {'image': image_binary}
        response_api = await sync_op(cls, ApiEndpoint(path='/proxy/stability/v2beta/stable-image/upscale/fast', method='POST'), response_model=StabilityStableUltraResponse, files=files, content_type='multipart/form-data')
        if response_api.finish_reason != 'SUCCESS':
...
class StabilityTextToAudio_ISO(IO.ComfyNode):
    ...
    async def execute(cls, model: str, prompt: str, duration: int, seed: int, steps: int) -> IO.NodeOutput:
        ...
        response_api = await sync_op(cls, ApiEndpoint(path='/proxy/stability/v2beta/audio/stable-audio-2/text-to-audio', method='POST'), response_model=StabilityAudioResponse, data=payload, content_type='multipart/form-data')
...
class StabilityAudioToAudio_ISO(IO.ComfyNode):
    ...
    @classmethod
    def define_schema(cls):
        return IO.Schema(node_id='StabilityAudioToAudio_ISO', display_name='Stability AI Audio To Audio_ISO', category='api node/audio/Stability AI', description=cleandoc(cls.__doc__ or ''), inputs=[IO.Combo.Input('model', options=['stable-audio-2.5']), IO.String.Input('prompt', multiline=True, default=''), IO.Audio.Input('audio', tooltip='Audio must be between 6 and 190 seconds long.'), IO.Int.Input('duration', default=190, min=1, max=190, step=1, tooltip='Controls the duration in seconds of the generated audio.', optional=True), IO.Int.Input('seed', default=0, min=0, max=4294967294, step=1, display_mode=IO.NumberDisplay.number, control_after_generate=True, tooltip='The random seed used for generation.', optional=True), IO.Int.Input('steps', default=8, min=4, max=8, step=1, tooltip='Controls the number of sampling steps.', optional=True), IO.Float.Input('strength', default=1, min=0.01, max=1.0, step=0.01, display_mode=IO.NumberDisplay.slider, tooltip='Parameter controls how much influence the audio parameter has on the generated audio.', optional=True)], outputs=[IO.Audio.Output()], hidden=[IO.Hidden.auth_token_comfy_org, IO.Hidden.api_key_comfy_org, IO.Hidden.unique_id], is_api_node=True)

    @classmethod
    async def execute(cls, model: str, prompt: str, audio: Input.Audio, duration: int, seed: int, steps: int, strength: float) -> IO.NodeOutput:
        validate_string(prompt, max_length=10000)
        validate_audio_duration(audio, 6, 190)
        payload = StabilityAudioToAudioRequest(prompt=prompt, model=model, duration=duration, seed=seed, steps=steps, strength=strength)
        response_api = await sync_op(cls, ApiEndpoint(path='/proxy/stability/v2beta/audio/stable-audio-2/audio-to-audio', method='POST'), response_model=StabilityAudioResponse, data=payload, content_type='multipart/form-data', files={'audio': audio_input_to_mp3(audio)})
        if not response_api.audio:
            raise ValueError('No audio file was received in response.')
        return IO.NodeOutput(audio_bytes_to_audio_input(base64.b64decode(response_api.audio)))
...
class StabilityAudioInpaint_ISO(IO.ComfyNode):
    ...
    @classmethod
    def define_schema(cls):
        return IO.Schema(node_id='StabilityAudioInpaint_ISO', display_name='Stability AI Audio Inpaint_ISO', category='api node/audio/Stability AI', description=cleandoc(cls.__doc__ or ''), inputs=[IO.Combo.Input('model', options=['stable-audio-2.5']), IO.String.Input('prompt', multiline=True, default=''), IO.Audio.Input('audio', tooltip='Audio must be between 6 and 190 seconds long.'), IO.Int.Input('duration', default=190, min=1, max=190, step=1, tooltip='Controls the duration in seconds of the generated audio.', optional=True), IO.Int.Input('seed', default=0, min=0, max=4294967294, step=1, display_mode=IO.NumberDisplay.number, control_after_generate=True, tooltip='The random seed used for generation.', optional=True), IO.Int.Input('steps', default=8, min=4, max=8, step=1, tooltip='Controls the number of sampling steps.', optional=True), IO.Int.Input('mask_start', default=30, min=0, max=190, step=1, optional=True), IO.Int.Input('mask_end', default=190, min=0, max=190, step=1, optional=True)], outputs=[IO.Audio.Output()], hidden=[IO.Hidden.auth_token_comfy_org, IO.Hidden.api_key_comfy_org, IO.Hidden.unique_id], is_api_node=True)

    @classmethod
    async def execute(cls, model: str, prompt: str, audio: Input.Audio, duration: int, seed: int, steps: int, mask_start: int, mask_end: int) -> IO.NodeOutput:
        validate_string(prompt, max_length=10000)
        if mask_end <= mask_start:
            raise ValueError(f'Value of mask_end({mask_end}) should be greater then mask_start({mask_start})')
        validate_audio_duration(audio, 6, 190)
        payload = StabilityAudioInpaintRequest(prompt=prompt, model=model, duration=duration, seed=seed, steps=steps, mask_start=mask_start, mask_end=mask_end)
        response_api = await sync_op(cls, endpoint=ApiEndpoint(path='/proxy/stability/v2beta/audio/stable-audio-2/inpaint', method='POST'), response_model=StabilityAudioResponse, data=payload, content_type='multipart/form-data', files={'audio': audio_input_to_mp3(audio)})
        if not response_api.audio:
            raise ValueError('No audio file was received in response.')
        return IO.NodeOutput(audio_bytes_to_audio_input(base64.b64decode(response_api.audio)))

class StabilityExtension_ISO(ComfyExtension):

    @override
    async def get_node_list(self) -> list[type[IO.ComfyNode]]:
        return [StabilityStableImageUltraNode_ISO, StabilityStableImageSD_3_5Node_ISO, StabilityUpscaleConservativeNode_ISO, StabilityUpscaleCreativeNode_ISO, StabilityUpscaleFastNode_ISO, StabilityTextToAudio_ISO, StabilityAudioToAudio_ISO, StabilityAudioInpaint_ISO]

async def comfy_entrypoint() -> StabilityExtension_ISO:
    return StabilityExtension_ISO()