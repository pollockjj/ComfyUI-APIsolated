...
class VeoVideoGenerationNode_ISO(IO.ComfyNode):
    ...
    async def execute(cls, prompt, aspect_ratio='16:9', negative_prompt='', duration_seconds=5, enhance_prompt=True, person_generation='ALLOW', seed=0, image=None, model='veo-2.0-generate-001', generate_audio=False):
        ...
        parameters = {'aspectRatio': aspect_ratio, 'personGeneration': person_generation, 'durationSeconds': duration_seconds, 'enhancePrompt': enhance_prompt}
        if negative_prompt:
        ...
        initial_response = await sync_op(cls, ApiEndpoint(path=f'/proxy/veo/{model}/generate', method='POST'), response_model=VeoGenVidResponse, data=VeoGenVidRequest(instances=instances, parameters=parameters))

        def status_extractor(response):
            return 'completed' if response.done else 'pending'
        poll_response = await poll_op(cls, ApiEndpoint(path=f'/proxy/veo/{model}/poll', method='POST'), response_model=VeoGenVidPollResponse, status_extractor=status_extractor, data=VeoGenVidPollRequest(operationName=initial_response.name), poll_interval=5.0, estimated_duration=AVERAGE_DURATION_VIDEO_GEN)
        if poll_response.error:
            raise Exception(f'Veo API error: {poll_response.error.message} (code: {poll_response.error.code})')
        if hasattr(poll_response.response, 'raiMediaFilteredCount') and poll_response.response.raiMediaFilteredCount > 0:
            if hasattr(poll_response.response, 'raiMediaFilteredReasons') and poll_response.response.raiMediaFilteredReasons:
...
class Veo3VideoGenerationNode(VeoVideoGenerationNode_ISO):
    """
...
class Veo3FirstLastFrameNode_ISO(IO.ComfyNode):
    ...
    async def execute(cls, prompt: str, negative_prompt: str, resolution: str, aspect_ratio: str, duration: int, seed: int, first_frame: torch.Tensor, last_frame: torch.Tensor, model: str, generate_audio: bool):
        ...
        if filtered_count:
            ...
            raise Exception(f"Content blocked by Google's Responsible AI filters{reason_part} ({filtered_count} video{('s' if filtered_count != 1 else '')} filtered).")
        if response.videos:
...
class VeoExtension_ISO(ComfyExtension):

    @override
    async def get_node_list(self) -> list[type[IO.ComfyNode]]:
        return [VeoVideoGenerationNode_ISO, Veo3VideoGenerationNode, Veo3FirstLastFrameNode_ISO]

async def comfy_entrypoint() -> VeoExtension_ISO:
    return VeoExtension_ISO()