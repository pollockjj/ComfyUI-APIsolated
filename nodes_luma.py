...
from comfy_api_nodes.apis.luma_api import LumaAspectRatio, LumaCharacterRef, LumaConceptChain, LumaGeneration, LumaGenerationRequest, LumaImageGenerationRequest, LumaImageIdentity, LumaImageModel, LumaImageReference, LumaIO, LumaKeyframes, LumaModifyImageRef, LumaReference, LumaReferenceChain, LumaVideoModel, LumaVideoModelOutputDuration, LumaVideoOutputResolution, get_luma_concepts
from comfy_api_nodes.util import ApiEndpoint, download_url_to_image_tensor, download_url_to_video_output, poll_op, sync_op, upload_images_to_comfyapi, validate_string
...
class LumaImageGenerationNode_ISO(IO.ComfyNode):
    ...
    async def execute(cls, prompt: str, model: str, aspect_ratio: str, seed, style_image_weight: float, image_luma_ref: Optional[LumaReferenceChain]=None, style_image: Optional[torch.Tensor]=None, character_image: Optional[torch.Tensor]=None) -> IO.NodeOutput:
        ...
        response_api = await sync_op(cls, ApiEndpoint(path='/proxy/luma/generations/image', method='POST'), response_model=LumaGeneration, data=LumaImageGenerationRequest(prompt=prompt, model=model, aspect_ratio=aspect_ratio, image_ref=api_image_ref, style_ref=api_style_ref, character_ref=character_ref))
        response_poll = await poll_op(cls, ApiEndpoint(path=f'/proxy/luma/generations/{response_api.id}'), response_model=LumaGeneration, status_extractor=lambda x: x.state)
...
class LumaImageModifyNode_ISO(IO.ComfyNode):
    ...
    @classmethod
    def define_schema(cls) -> IO.Schema:
        return IO.Schema(node_id='LumaImageModifyNode_ISO', display_name='Luma Image to Image_ISO', category='api node/image/Luma', description='Modifies images synchronously based on prompt and aspect ratio.', inputs=[IO.Image.Input('image'), IO.String.Input('prompt', multiline=True, default='', tooltip='Prompt for the image generation'), IO.Float.Input('image_weight', default=0.1, min=0.0, max=0.98, step=0.01, tooltip='Weight of the image; the closer to 1.0, the less the image will be modified.'), IO.Combo.Input('model', options=LumaImageModel), IO.Int.Input('seed', default=0, min=0, max=18446744073709551615, control_after_generate=True, tooltip='Seed to determine if node should re-run; actual results are nondeterministic regardless of seed.')], outputs=[IO.Image.Output()], hidden=[IO.Hidden.auth_token_comfy_org, IO.Hidden.api_key_comfy_org, IO.Hidden.unique_id], is_api_node=True)

    @classmethod
    async def execute(cls, prompt: str, model: str, image: torch.Tensor, image_weight: float, seed) -> IO.NodeOutput:
        download_urls = await upload_images_to_comfyapi(cls, image, max_images=1)
        image_url = download_urls[0]
        response_api = await sync_op(cls, ApiEndpoint(path='/proxy/luma/generations/image', method='POST'), response_model=LumaGeneration, data=LumaImageGenerationRequest(prompt=prompt, model=model, modify_image_ref=LumaModifyImageRef(url=image_url, weight=round(max(min(1.0 - image_weight, 0.98), 0.0), 2))))
        response_poll = await poll_op(cls, ApiEndpoint(path=f'/proxy/luma/generations/{response_api.id}'), response_model=LumaGeneration, status_extractor=lambda x: x.state)
        return IO.NodeOutput(await download_url_to_image_tensor(response_poll.assets.image))
...
class LumaTextToVideoGenerationNode_ISO(IO.ComfyNode):
    ...
    @classmethod
    def define_schema(cls) -> IO.Schema:
        return IO.Schema(node_id='LumaVideoNode_ISO', display_name='Luma Text to Video_ISO', category='api node/video/Luma', description='Generates videos synchronously based on prompt and output_size.', inputs=[IO.String.Input('prompt', multiline=True, default='', tooltip='Prompt for the video generation'), IO.Combo.Input('model', options=LumaVideoModel), IO.Combo.Input('aspect_ratio', options=LumaAspectRatio, default=LumaAspectRatio.ratio_16_9), IO.Combo.Input('resolution', options=LumaVideoOutputResolution, default=LumaVideoOutputResolution.res_540p), IO.Combo.Input('duration', options=LumaVideoModelOutputDuration), IO.Boolean.Input('loop', default=False), IO.Int.Input('seed', default=0, min=0, max=18446744073709551615, control_after_generate=True, tooltip='Seed to determine if node should re-run; actual results are nondeterministic regardless of seed.'), IO.Custom(LumaIO.LUMA_CONCEPTS).Input('luma_concepts', tooltip='Optional Camera Concepts to dictate camera motion via the Luma Concepts node.', optional=True)], outputs=[IO.Video.Output()], hidden=[IO.Hidden.auth_token_comfy_org, IO.Hidden.api_key_comfy_org, IO.Hidden.unique_id], is_api_node=True)

    @classmethod
    async def execute(cls, prompt: str, model: str, aspect_ratio: str, resolution: str, duration: str, loop: bool, seed, luma_concepts: Optional[LumaConceptChain]=None) -> IO.NodeOutput:
        validate_string(prompt, strip_whitespace=False, min_length=3)
        duration = duration if model != LumaVideoModel.ray_1_6 else None
        resolution = resolution if model != LumaVideoModel.ray_1_6 else None
        response_api = await sync_op(cls, ApiEndpoint(path='/proxy/luma/generations', method='POST'), response_model=LumaGeneration, data=LumaGenerationRequest(prompt=prompt, model=model, resolution=resolution, aspect_ratio=aspect_ratio, duration=duration, loop=loop, concepts=luma_concepts.create_api_model() if luma_concepts else None))
        response_poll = await poll_op(cls, ApiEndpoint(path=f'/proxy/luma/generations/{response_api.id}'), response_model=LumaGeneration, status_extractor=lambda x: x.state, estimated_duration=LUMA_T2V_AVERAGE_DURATION)
...
class LumaImageToVideoGenerationNode_ISO(IO.ComfyNode):
    ...
    @classmethod
    def define_schema(cls) -> IO.Schema:
        return IO.Schema(node_id='LumaImageToVideoNode_ISO', display_name='Luma Image to Video_ISO', category='api node/video/Luma', description='Generates videos synchronously based on prompt, input images, and output_size.', inputs=[IO.String.Input('prompt', multiline=True, default='', tooltip='Prompt for the video generation'), IO.Combo.Input('model', options=LumaVideoModel), IO.Combo.Input('resolution', options=LumaVideoOutputResolution, default=LumaVideoOutputResolution.res_540p), IO.Combo.Input('duration', options=[dur.value for dur in LumaVideoModelOutputDuration]), IO.Boolean.Input('loop', default=False), IO.Int.Input('seed', default=0, min=0, max=18446744073709551615, control_after_generate=True, tooltip='Seed to determine if node should re-run; actual results are nondeterministic regardless of seed.'), IO.Image.Input('first_image', tooltip='First frame of generated video.', optional=True), IO.Image.Input('last_image', tooltip='Last frame of generated video.', optional=True), IO.Custom(LumaIO.LUMA_CONCEPTS).Input('luma_concepts', tooltip='Optional Camera Concepts to dictate camera motion via the Luma Concepts node.', optional=True)], outputs=[IO.Video.Output()], hidden=[IO.Hidden.auth_token_comfy_org, IO.Hidden.api_key_comfy_org, IO.Hidden.unique_id], is_api_node=True)

    @classmethod
    async def execute(cls, prompt: str, model: str, resolution: str, duration: str, loop: bool, seed, first_image: torch.Tensor=None, last_image: torch.Tensor=None, luma_concepts: LumaConceptChain=None) -> IO.NodeOutput:
        ...
        duration = duration if model != LumaVideoModel.ray_1_6 else None
        resolution = resolution if model != LumaVideoModel.ray_1_6 else None
        response_api = await sync_op(cls, ApiEndpoint(path='/proxy/luma/generations', method='POST'), response_model=LumaGeneration, data=LumaGenerationRequest(prompt=prompt, model=model, aspect_ratio=LumaAspectRatio.ratio_16_9, resolution=resolution, duration=duration, loop=loop, keyframes=keyframes, concepts=luma_concepts.create_api_model() if luma_concepts else None))
        response_poll = await poll_op(cls, poll_endpoint=ApiEndpoint(path=f'/proxy/luma/generations/{response_api.id}'), response_model=LumaGeneration, status_extractor=lambda x: x.state, estimated_duration=LUMA_I2V_AVERAGE_DURATION)
        return IO.NodeOutput(await download_url_to_video_output(response_poll.assets.video))

    @classmethod
    async def _convert_to_keyframes(cls, first_image: torch.Tensor=None, last_image: torch.Tensor=None):
        if first_image is None and last_image is None:
            return None
        frame0 = None
        frame1 = None
...
class LumaExtension_ISO(ComfyExtension):

    @override
    async def get_node_list(self) -> list[type[IO.ComfyNode]]:
        return [LumaImageGenerationNode_ISO, LumaImageModifyNode_ISO, LumaTextToVideoGenerationNode_ISO, LumaImageToVideoGenerationNode_ISO, LumaReferenceNode_ISO, LumaConceptsNode_ISO]

async def comfy_entrypoint() -> LumaExtension_ISO:
    return LumaExtension_ISO()