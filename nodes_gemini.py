...
async def create_image_parts(cls: type[IO.ComfyNode], images: torch.Tensor, image_limit: int=0) -> list[GeminiPart]:
    ...
    reference_images_urls = await upload_images_to_comfyapi(cls, images, max_images=num_url_images)
    for reference_image_url in reference_images_urls:
        image_parts.append(GeminiPart(fileData=GeminiFileData(mimeType=GeminiMimeType.image_png, fileUri=reference_image_url)))
    for idx in range(num_url_images, effective_max):
        image_parts.append(GeminiPart(inlineData=GeminiInlineData(mimeType=GeminiMimeType.image_png, data=tensor_to_base64_string(images[idx]))))
    return image_parts
...
def get_parts_by_type(response: GeminiGenerateContentResponse, part_type: Literal['text'] | str) -> list[GeminiPart]:
    ...
    if response.candidates is None:
        if response.promptFeedback and response.promptFeedback.blockReason:
            ...
            raise ValueError(f'Gemini API blocked the request. Reason: {feedback.blockReason} ({feedback.blockReasonMessage})')
        raise ValueError("Gemini API returned no response candidates. If you are using the `IMAGE` modality, try changing it to `IMAGE+TEXT` to view the model's reasoning and understand why image generation failed.")
    parts = []
...
def calculate_tokens_price(response: GeminiGenerateContentResponse) -> float | None:
    ...
    elif response.modelVersion in ('gemini-2.5-flash-preview-04-17', 'gemini-2.5-flash'):
        input_tokens_price = 0.3
        output_text_tokens_price = 2.5
        output_image_tokens_price = 0.0
    elif response.modelVersion in ('gemini-2.5-flash-image-preview', 'gemini-2.5-flash-image'):
        input_tokens_price = 0.3
        output_text_tokens_price = 2.5
...
class GeminiNode_ISO(IO.ComfyNode):
    ...
    def create_video_parts(cls, video_input: Input.Video) -> list[GeminiPart]:
        ...
        return [GeminiPart(inlineData=GeminiInlineData(mimeType=GeminiMimeType.video_mp4, data=base_64_string))]
    ...
    def create_audio_parts(cls, audio_input: Input.Audio) -> list[GeminiPart]:
        ...
        for batch_index in range(audio_input['waveform'].shape[0]):
            audio_at_index = Input.Audio(waveform=audio_input['waveform'][batch_index].unsqueeze(0), sample_rate=audio_input['sample_rate'])
            audio_bytes = audio_to_base64_string(audio_at_index, container_format='mp3', codec_name='libmp3lame')
            audio_parts.append(GeminiPart(inlineData=GeminiInlineData(mimeType=GeminiMimeType.audio_mp3, data=audio_bytes)))
        return audio_parts

    @classmethod
    async def execute(cls, prompt: str, model: str, seed: int, images: torch.Tensor | None=None, audio: Input.Audio | None=None, video: Input.Video | None=None, files: list[GeminiPart] | None=None) -> IO.NodeOutput:
        ...
        if files is not None:
            parts.extend(files)
        response = await sync_op(cls, endpoint=ApiEndpoint(path=f'{GEMINI_BASE_ENDPOINT}/{model}', method='POST'), data=GeminiGenerateContentRequest(contents=[GeminiContent(role=GeminiRole.user, parts=parts)]), response_model=GeminiGenerateContentResponse, price_extractor=calculate_tokens_price)
...
class GeminiInputFiles_ISO(IO.ComfyNode):
    ...
    def create_file_part(cls, file_path: str) -> GeminiPart:
        ...
        return GeminiPart(inlineData=GeminiInlineData(mimeType=mime_type, data=base64_str))
...
class GeminiImage_ISO(IO.ComfyNode):
    ...
    async def execute(cls, prompt: str, model: str, seed: int, images: torch.Tensor | None=None, files: list[GeminiPart] | None=None, aspect_ratio: str='auto', response_modalities: str='IMAGE+TEXT') -> IO.NodeOutput:
        ...
        if files is not None:
            parts.extend(files)
        response = await sync_op(cls, endpoint=ApiEndpoint(path=f'{GEMINI_BASE_ENDPOINT}/{model}', method='POST'), data=GeminiImageGenerateContentRequest(contents=[GeminiContent(role=GeminiRole.user, parts=parts)], generationConfig=GeminiImageGenerationConfig(responseModalities=['IMAGE'] if response_modalities == 'IMAGE' else ['TEXT', 'IMAGE'], imageConfig=None if aspect_ratio == 'auto' else image_config)), response_model=GeminiGenerateContentResponse, price_extractor=calculate_tokens_price)
...
class GeminiExtension_ISO(ComfyExtension):

    @override
    async def get_node_list(self) -> list[type[IO.ComfyNode]]:
        return [GeminiNode_ISO, GeminiImage_ISO, GeminiImage2_ISO, GeminiInputFiles_ISO]

async def comfy_entrypoint() -> GeminiExtension_ISO:
    return GeminiExtension_ISO()