...
class FluxProFillNode_ISO(IO.ComfyNode):
    ...
    @classmethod
    def define_schema(cls) -> IO.Schema:
        return IO.Schema(node_id='FluxProFillNode_ISO', display_name='Flux.1 Fill Image_ISO', category='api node/image/BFL', description=cleandoc(cls.__doc__ or ''), inputs=[IO.Image.Input('image'), IO.Mask.Input('mask'), IO.String.Input('prompt', multiline=True, default='', tooltip='Prompt for the image generation'), IO.Boolean.Input('prompt_upsampling', default=False, tooltip='Whether to perform upsampling on the prompt. If active, automatically modifies the prompt for more creative generation, but results are nondeterministic (same seed will not produce exactly the same result).'), IO.Float.Input('guidance', default=60, min=1.5, max=100, tooltip='Guidance strength for the image generation process'), IO.Int.Input('steps', default=50, min=15, max=50, tooltip='Number of steps for the image generation process'), IO.Int.Input('seed', default=0, min=0, max=18446744073709551615, control_after_generate=True, tooltip='The random seed used for creating the noise.')], outputs=[IO.Image.Output()], hidden=[IO.Hidden.auth_token_comfy_org, IO.Hidden.api_key_comfy_org, IO.Hidden.unique_id], is_api_node=True)

    @classmethod
    async def execute(cls, image: torch.Tensor, mask: torch.Tensor, prompt: str, prompt_upsampling: bool, steps: int, guidance: float, seed=0) -> IO.NodeOutput:
        mask = resize_mask_to_image(mask, image)
        mask = tensor_to_base64_string(convert_mask_to_image(mask))
        initial_response = await sync_op(cls, ApiEndpoint(path='/proxy/bfl/flux-pro-1.0-fill/generate', method='POST'), response_model=BFLFluxProGenerateResponse, data=BFLFluxFillImageRequest(prompt=prompt, prompt_upsampling=prompt_upsampling, steps=steps, guidance=guidance, seed=seed, image=tensor_to_base64_string(image[:, :, :, :3]), mask=mask))
        response = await poll_op(cls, ApiEndpoint(initial_response.polling_url), response_model=BFLFluxStatusResponse, status_extractor=lambda r: r.status, progress_extractor=lambda r: r.progress, completed_statuses=[BFLStatus.ready], failed_statuses=[BFLStatus.request_moderated, BFLStatus.content_moderated, BFLStatus.error, BFLStatus.task_not_found], queued_statuses=[])
        return IO.NodeOutput(await download_url_to_image_tensor(response.result['sample']))
...
class Flux2ProImageNode_ISO(IO.ComfyNode):
    ...
    async def execute(cls, prompt: str, width: int, height: int, seed: int, prompt_upsampling: bool, images: torch.Tensor | None=None) -> IO.NodeOutput:
        ...
        initial_response = await sync_op(cls, ApiEndpoint(path='/proxy/bfl/flux-2-pro/generate', method='POST'), response_model=BFLFluxProGenerateResponse, data=Flux2ProGenerateRequest(prompt=prompt, width=width, height=height, seed=seed, prompt_upsampling=prompt_upsampling, **reference_images))
...
class BFLExtension_ISO(ComfyExtension):

    @override
    async def get_node_list(self) -> list[type[IO.ComfyNode]]:
        return [FluxProUltraImageNode_ISO, FluxKontextProImageNode_ISO, FluxKontextMaxImageNode, FluxProExpandNode_ISO, FluxProFillNode_ISO, Flux2ProImageNode_ISO]

async def comfy_entrypoint() -> BFLExtension_ISO:
    return BFLExtension_ISO()