...
class TopazVideoEnhance_ISO(IO.ComfyNode):
    ...
    async def execute(cls, video: Input.Video, upscaler_enabled: bool, upscaler_model: str, upscaler_resolution: str, upscaler_creativity: str='low', interpolation_enabled: bool=False, interpolation_model: str='apo-8', interpolation_slowmo: int=1, interpolation_frame_rate: int=60, interpolation_duplicate: bool=False, interpolation_duplicate_threshold: float=0.01, dynamic_compression_level: str='Low') -> IO.NodeOutput:
        ...
        if upscaler_enabled:
            ...
            filters.append(topaz_api.VideoEnhancementFilter(model=UPSCALER_MODELS_MAP[upscaler_model], creativity=upscaler_creativity if UPSCALER_MODELS_MAP[upscaler_model] == 'slc-1' else None, isOptimizedMode=True if UPSCALER_MODELS_MAP[upscaler_model] == 'slc-1' else None))
        if interpolation_enabled:
            target_frame_rate = interpolation_frame_rate
            filters.append(topaz_api.VideoFrameInterpolationFilter(model=interpolation_model, slowmo=interpolation_slowmo, fps=interpolation_frame_rate, duplicate=interpolation_duplicate, duplicate_threshold=interpolation_duplicate_threshold))
        initial_res = await sync_op(cls, ApiEndpoint(path='/proxy/topaz/video/', method='POST'), response_model=topaz_api.CreateVideoResponse, data=topaz_api.CreateVideoRequest(source=topaz_api.CreateCreateVideoRequestSource(container='mp4', size=get_fs_object_size(src_video_stream), duration=int(duration_sec), frameCount=video.get_frame_count(), frameRate=src_frame_rate, resolution=topaz_api.Resolution(width=src_width, height=src_height)), filters=filters, output=topaz_api.OutputInformationVideo(resolution=topaz_api.Resolution(width=target_width, height=target_height), frameRate=target_frame_rate, audioCodec='AAC', audioTransfer='Copy', dynamicCompressionLevel=dynamic_compression_level)), wait_label='Creating task', final_label_on_success='Task created')
        upload_res = await sync_op(cls, ApiEndpoint(path=f'/proxy/topaz/video/{initial_res.requestId}/accept', method='PATCH'), response_model=topaz_api.VideoAcceptResponse, wait_label='Preparing upload', final_label_on_success='Upload started')
        if len(upload_res.urls) > 1:
            raise NotImplementedError('Large files are not currently supported. Please open an issue in the ComfyUI repository.')
            ...
            if isinstance(src_video_stream, BytesIO):
                ...
                with builtins.open(src_video_stream, 'rb') as video_file:
                    async with session.put(upload_res.urls[0], data=video_file, raise_for_status=True) as res:
                        upload_etag = res.headers['Etag']
        await sync_op(cls, ApiEndpoint(path=f'/proxy/topaz/video/{initial_res.requestId}/complete-upload', method='PATCH'), response_model=topaz_api.VideoCompleteUploadResponse, data=topaz_api.VideoCompleteUploadRequest(uploadResults=[topaz_api.VideoCompleteUploadRequestPart(partNum=1, eTag=upload_etag)]), wait_label='Finalizing upload', final_label_on_success='Upload completed')
        final_response = await poll_op(cls, ApiEndpoint(path=f'/proxy/topaz/video/{initial_res.requestId}/status'), response_model=topaz_api.VideoStatusResponse, status_extractor=lambda x: x.status, progress_extractor=lambda x: getattr(x, 'progress', 0), price_extractor=lambda x: x.estimates.cost[0] * 0.08 if x.estimates and x.estimates.cost[0] else None, poll_interval=10.0, max_poll_attempts=320)
        return IO.NodeOutput(await download_url_to_video_output(final_response.download.url))

class TopazExtension_ISO(ComfyExtension):

    @override
    async def get_node_list(self) -> list[type[IO.ComfyNode]]:
        return [TopazImageEnhance_ISO, TopazVideoEnhance_ISO]

async def comfy_entrypoint() -> TopazExtension_ISO:
    return TopazExtension_ISO()