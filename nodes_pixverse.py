...
async def upload_image_to_pixverse(cls: type[IO.ComfyNode], image: torch.Tensor):
    response_upload = await sync_op(cls, ApiEndpoint(path='/proxy/pixverse/image/upload', method='POST'), response_model=PixverseImageUploadResponse, files={'image': tensor_to_bytesio(image)}, content_type='multipart/form-data')
...
class PixverseTemplateNode_ISO(IO.ComfyNode):
    ...
    def define_schema(cls) -> IO.Schema:
        return IO.Schema(node_id='PixverseTemplateNode_ISO', display_name='PixVerse Template_ISO', category='api node/video/PixVerse', inputs=[IO.Combo.Input('template', options=list(pixverse_templates.keys()))], outputs=[IO.Custom(PixverseIO.TEMPLATE).Output(display_name='pixverse_template')])
...
class PixverseImageToVideoNode_ISO(IO.ComfyNode):
    ...
    @classmethod
    def define_schema(cls) -> IO.Schema:
        return IO.Schema(node_id='PixverseImageToVideoNode_ISO', display_name='PixVerse Image to Video_ISO', category='api node/video/PixVerse', description='Generates videos based on prompt and output_size.', inputs=[IO.Image.Input('image'), IO.String.Input('prompt', multiline=True, default='', tooltip='Prompt for the video generation'), IO.Combo.Input('quality', options=PixverseQuality, default=PixverseQuality.res_540p), IO.Combo.Input('duration_seconds', options=PixverseDuration), IO.Combo.Input('motion_mode', options=PixverseMotionMode), IO.Int.Input('seed', default=0, min=0, max=2147483647, control_after_generate=True, tooltip='Seed for video generation.'), IO.String.Input('negative_prompt', default='', multiline=True, tooltip='An optional text description of undesired elements on an image.', optional=True), IO.Custom(PixverseIO.TEMPLATE).Input('pixverse_template', tooltip='An optional template to influence style of generation, created by the PixVerse Template node.', optional=True)], outputs=[IO.Video.Output()], hidden=[IO.Hidden.auth_token_comfy_org, IO.Hidden.api_key_comfy_org, IO.Hidden.unique_id], is_api_node=True)

    @classmethod
    async def execute(cls, image: torch.Tensor, prompt: str, quality: str, duration_seconds: int, motion_mode: str, seed, negative_prompt: str=None, pixverse_template: int=None) -> IO.NodeOutput:
        ...
        if quality == PixverseQuality.res_1080p:
            ...
            motion_mode = PixverseMotionMode.normal
        response_api = await sync_op(cls, ApiEndpoint(path='/proxy/pixverse/video/img/generate', method='POST'), response_model=PixverseVideoResponse, data=PixverseImageVideoRequest(img_id=img_id, prompt=prompt, quality=quality, duration=duration_seconds, motion_mode=motion_mode, negative_prompt=negative_prompt if negative_prompt else None, template_id=pixverse_template, seed=seed))
        if response_api.Resp is None:
            raise Exception(f"PixVerse request failed: '{response_api.ErrMsg}'")
        response_poll = await poll_op(cls, ApiEndpoint(path=f'/proxy/pixverse/video/result/{response_api.Resp.video_id}'), response_model=PixverseGenerationStatusResponse, completed_statuses=[PixverseStatus.successful], failed_statuses=[PixverseStatus.contents_moderation, PixverseStatus.failed, PixverseStatus.deleted], status_extractor=lambda x: x.Resp.status, estimated_duration=AVERAGE_DURATION_I2V)
        return IO.NodeOutput(await download_url_to_video_output(response_poll.Resp.url))
...
class PixverseTransitionVideoNode_ISO(IO.ComfyNode):
    ...
    @classmethod
    def define_schema(cls) -> IO.Schema:
        return IO.Schema(node_id='PixverseTransitionVideoNode_ISO', display_name='PixVerse Transition Video_ISO', category='api node/video/PixVerse', description='Generates videos based on prompt and output_size.', inputs=[IO.Image.Input('first_frame'), IO.Image.Input('last_frame'), IO.String.Input('prompt', multiline=True, default='', tooltip='Prompt for the video generation'), IO.Combo.Input('quality', options=PixverseQuality, default=PixverseQuality.res_540p), IO.Combo.Input('duration_seconds', options=PixverseDuration), IO.Combo.Input('motion_mode', options=PixverseMotionMode), IO.Int.Input('seed', default=0, min=0, max=2147483647, control_after_generate=True, tooltip='Seed for video generation.'), IO.String.Input('negative_prompt', default='', multiline=True, tooltip='An optional text description of undesired elements on an image.', optional=True)], outputs=[IO.Video.Output()], hidden=[IO.Hidden.auth_token_comfy_org, IO.Hidden.api_key_comfy_org, IO.Hidden.unique_id], is_api_node=True)

    @classmethod
    async def execute(cls, first_frame: torch.Tensor, last_frame: torch.Tensor, prompt: str, quality: str, duration_seconds: int, motion_mode: str, seed, negative_prompt: str=None) -> IO.NodeOutput:
        validate_string(prompt, strip_whitespace=False)
        ...
        if quality == PixverseQuality.res_1080p:
            ...
            motion_mode = PixverseMotionMode.normal
        response_api = await sync_op(cls, ApiEndpoint(path='/proxy/pixverse/video/transition/generate', method='POST'), response_model=PixverseVideoResponse, data=PixverseTransitionVideoRequest(first_frame_img=first_frame_id, last_frame_img=last_frame_id, prompt=prompt, quality=quality, duration=duration_seconds, motion_mode=motion_mode, negative_prompt=negative_prompt if negative_prompt else None, seed=seed))
        if response_api.Resp is None:
            raise Exception(f"PixVerse request failed: '{response_api.ErrMsg}'")
        response_poll = await poll_op(cls, ApiEndpoint(path=f'/proxy/pixverse/video/result/{response_api.Resp.video_id}'), response_model=PixverseGenerationStatusResponse, completed_statuses=[PixverseStatus.successful], failed_statuses=[PixverseStatus.contents_moderation, PixverseStatus.failed, PixverseStatus.deleted], status_extractor=lambda x: x.Resp.status, estimated_duration=AVERAGE_DURATION_T2V)
        return IO.NodeOutput(await download_url_to_video_output(response_poll.Resp.url))

class PixVerseExtension_ISO(ComfyExtension):

    @override
    async def get_node_list(self) -> list[type[IO.ComfyNode]]:
        return [PixverseTextToVideoNode_ISO, PixverseImageToVideoNode_ISO, PixverseTransitionVideoNode_ISO, PixverseTemplateNode_ISO]

async def comfy_entrypoint() -> PixVerseExtension_ISO:
    return PixVerseExtension_ISO()