...
from comfy_api_nodes.util import ApiEndpoint, audio_to_base64_string, download_url_to_image_tensor, download_url_to_video_output, get_number_of_images, poll_op, sync_op, tensor_to_base64_string, validate_audio_duration
...
class WanTextToImageApi_ISO(IO.ComfyNode):
    ...
    async def execute(cls, model: str, prompt: str, negative_prompt: str='', width: int=1024, height: int=1024, seed: int=0, prompt_extend: bool=True, watermark: bool=True):
        ...
        response = await poll_op(cls, ApiEndpoint(path=f'/proxy/wan/api/v1/tasks/{initial_response.output.task_id}'), response_model=ImageTaskStatusResponse, status_extractor=lambda x: x.output.task_status, estimated_duration=9, poll_interval=3)
...
class WanImageToImageApi_ISO(IO.ComfyNode):
    ...
    async def execute(cls, model: str, image: torch.Tensor, prompt: str, negative_prompt: str='', seed: int=0, watermark: bool=True):
        ...
        initial_response = await sync_op(cls, ApiEndpoint(path='/proxy/wan/api/v1/services/aigc/image2image/image-synthesis', method='POST'), response_model=TaskCreationResponse, data=Image2ImageTaskCreationRequest(model=model, input=Image2ImageInputField(prompt=prompt, negative_prompt=negative_prompt, images=images), parameters=Image2ImageParametersField(seed=seed, watermark=watermark)))
        if not initial_response.output:
            raise Exception(f'Unknown error occurred: {initial_response.code} - {initial_response.message}')
        response = await poll_op(cls, ApiEndpoint(path=f'/proxy/wan/api/v1/tasks/{initial_response.output.task_id}'), response_model=ImageTaskStatusResponse, status_extractor=lambda x: x.output.task_status, estimated_duration=42, poll_interval=4)
        return IO.NodeOutput(await download_url_to_image_tensor(str(response.output.results[0].url)))
...
class WanTextToVideoApi_ISO(IO.ComfyNode):
    ...
    async def execute(cls, model: str, prompt: str, negative_prompt: str='', size: str='480p: 1:1 (624x624)', duration: int=5, audio: Optional[Input.Audio]=None, seed: int=0, generate_audio: bool=False, prompt_extend: bool=True, watermark: bool=True):
        ...
        if not initial_response.output:
            raise Exception(f'Unknown error occurred: {initial_response.code} - {initial_response.message}')
        response = await poll_op(cls, ApiEndpoint(path=f'/proxy/wan/api/v1/tasks/{initial_response.output.task_id}'), response_model=VideoTaskStatusResponse, status_extractor=lambda x: x.output.task_status, estimated_duration=120 * int(duration / 5), poll_interval=6)
...
class WanImageToVideoApi_ISO(IO.ComfyNode):
    ...
    @classmethod
    def define_schema(cls):
        return IO.Schema(node_id='WanImageToVideoApi_ISO', display_name='Wan Image to Video_ISO', category='api node/video/Wan', description='Generates video based on the first frame and text prompt.', inputs=[IO.Combo.Input('model', options=['wan2.5-i2v-preview'], default='wan2.5-i2v-preview', tooltip='Model to use.'), IO.Image.Input('image'), IO.String.Input('prompt', multiline=True, default='', tooltip='Prompt used to describe the elements and visual features, supports English/Chinese.'), IO.String.Input('negative_prompt', multiline=True, default='', tooltip='Negative text prompt to guide what to avoid.', optional=True), IO.Combo.Input('resolution', options=['480P', '720P', '1080P'], default='480P', optional=True), IO.Int.Input('duration', default=5, min=5, max=10, step=5, display_mode=IO.NumberDisplay.number, tooltip='Available durations: 5 and 10 seconds', optional=True), IO.Audio.Input('audio', optional=True, tooltip='Audio must contain a clear, loud voice, without extraneous noise, background music.'), IO.Int.Input('seed', default=0, min=0, max=2147483647, step=1, display_mode=IO.NumberDisplay.number, control_after_generate=True, tooltip='Seed to use for generation.', optional=True), IO.Boolean.Input('generate_audio', default=False, optional=True, tooltip='If there is no audio input, generate audio automatically.'), IO.Boolean.Input('prompt_extend', default=True, tooltip='Whether to enhance the prompt with AI assistance.', optional=True), IO.Boolean.Input('watermark', default=True, tooltip='Whether to add an "AI generated" watermark to the result.', optional=True)], outputs=[IO.Video.Output()], hidden=[IO.Hidden.auth_token_comfy_org, IO.Hidden.api_key_comfy_org, IO.Hidden.unique_id], is_api_node=True)

    @classmethod
    async def execute(cls, model: str, image: torch.Tensor, prompt: str, negative_prompt: str='', resolution: str='480P', duration: int=5, audio: Optional[Input.Audio]=None, seed: int=0, generate_audio: bool=False, prompt_extend: bool=True, watermark: bool=True):
...
class WanApiExtension_ISO(ComfyExtension):

    @override
    async def get_node_list(self) -> list[type[IO.ComfyNode]]:
        return [WanTextToImageApi_ISO, WanImageToImageApi_ISO, WanTextToVideoApi_ISO, WanImageToVideoApi_ISO]

async def comfy_entrypoint() -> WanApiExtension_ISO:
    return WanApiExtension_ISO()