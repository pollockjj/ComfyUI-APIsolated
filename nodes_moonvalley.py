...
from comfy_api_nodes.apis import MoonvalleyPromptResponse, MoonvalleyTextToVideoInferenceParams, MoonvalleyTextToVideoRequest, MoonvalleyVideoToVideoInferenceParams, MoonvalleyVideoToVideoRequest
from comfy_api_nodes.util import ApiEndpoint, download_url_to_video_output, poll_op, sync_op, trim_video, upload_images_to_comfyapi, upload_video_to_comfyapi, validate_container_format_is_mp4, validate_image_dimensions, validate_string
...
def _validate_video_dimensions(width: int, height: int) -> None:
    ...
    supported_resolutions = {(1920, 1080), (1080, 1920), (1152, 1152), (1536, 1152), (1152, 1536)}
...
def parse_width_height_from_res(resolution: str):
    res_map = {'16:9 (1920 x 1080)': {'width': 1920, 'height': 1080}, '9:16 (1080 x 1920)': {'width': 1080, 'height': 1920}, '1:1 (1152 x 1152)': {'width': 1152, 'height': 1152}, '4:3 (1536 x 1152)': {'width': 1536, 'height': 1152}, '3:4 (1152 x 1536)': {'width': 1152, 'height': 1536}}
    return res_map.get(resolution, {'width': 1920, 'height': 1080})

def parse_control_parameter(value):
    control_map = {'Motion Transfer': 'motion_control', 'Canny': 'canny_control', 'Pose Transfer': 'pose_control', 'Depth': 'depth_control'}
...
class MoonvalleyImg2VideoNode_ISO(IO.ComfyNode):
    ...
    async def execute(cls, image: torch.Tensor, prompt: str, negative_prompt: str, resolution: str, prompt_adherence: float, seed: int, steps: int) -> IO.NodeOutput:
        ...
        inference_params = MoonvalleyTextToVideoInferenceParams(negative_prompt=negative_prompt, steps=steps, seed=seed, guidance_scale=prompt_adherence, width=width_height['width'], height=width_height['height'], use_negative_prompts=True)
        mime_type = 'image/png'
        image_url = (await upload_images_to_comfyapi(cls, image, max_images=1, mime_type=mime_type))[0]
        task_creation_response = await sync_op(cls, endpoint=ApiEndpoint(path=API_IMG2VIDEO_ENDPOINT, method='POST'), response_model=MoonvalleyPromptResponse, data=MoonvalleyTextToVideoRequest(image_url=image_url, prompt_text=prompt, inference_params=inference_params))
...
class MoonvalleyVideo2VideoNode_ISO(IO.ComfyNode):
    ...
    async def execute(cls, prompt: str, negative_prompt: str, seed: int, video: Optional[VideoInput]=None, control_type: str='Motion Transfer', motion_intensity: Optional[int]=100, steps=33, prompt_adherence=4.5) -> IO.NodeOutput:
        ...
        inference_params = MoonvalleyVideoToVideoInferenceParams(negative_prompt=negative_prompt, seed=seed, control_params=control_params, steps=steps, guidance_scale=prompt_adherence)
        task_creation_response = await sync_op(cls, endpoint=ApiEndpoint(path=API_VIDEO2VIDEO_ENDPOINT, method='POST'), response_model=MoonvalleyPromptResponse, data=MoonvalleyVideoToVideoRequest(control_type=parse_control_parameter(control_type), video_url=video_url, prompt_text=prompt, inference_params=inference_params))
        validate_task_creation_response(task_creation_response)
        final_response = await get_response(cls, task_creation_response.id)
...
class MoonvalleyTxt2VideoNode_ISO(IO.ComfyNode):
    ...
    @classmethod
    def define_schema(cls) -> IO.Schema:
        return IO.Schema(node_id='MoonvalleyTxt2VideoNode_ISO', display_name='Moonvalley Marey Text to Video_ISO', category='api node/video/Moonvalley Marey', description='', inputs=[IO.String.Input('prompt', multiline=True), IO.String.Input('negative_prompt', multiline=True, default='<synthetic> <scene cut> gopro, bright, contrast, static, overexposed, vignette, artifacts, still, noise, texture, scanlines, videogame, 360 camera, VR, transition, flare, saturation, distorted, warped, wide angle, saturated, vibrant, glowing, cross dissolve, cheesy, ugly hands, mutated hands, mutant, disfigured, extra fingers, blown out, horrible, blurry, worst quality, bad, dissolve, melt, fade in, fade out, wobbly, weird, low quality, plastic, stock footage, video camera, boring', tooltip='Negative prompt text'), IO.Combo.Input('resolution', options=['16:9 (1920 x 1080)', '9:16 (1080 x 1920)', '1:1 (1152 x 1152)', '4:3 (1536 x 1152)', '3:4 (1152 x 1536)', '21:9 (2560 x 1080)'], default='16:9 (1920 x 1080)', tooltip='Resolution of the output video'), IO.Float.Input('prompt_adherence', default=4.0, min=1.0, max=20.0, step=1.0, tooltip='Guidance scale for generation control'), IO.Int.Input('seed', default=9, min=0, max=4294967295, step=1, display_mode=IO.NumberDisplay.number, control_after_generate=True, tooltip='Random seed value'), IO.Int.Input('steps', default=33, min=1, max=100, step=1, tooltip='Inference steps')], outputs=[IO.Video.Output()], hidden=[IO.Hidden.auth_token_comfy_org, IO.Hidden.api_key_comfy_org, IO.Hidden.unique_id], is_api_node=True)

    @classmethod
    async def execute(cls, prompt: str, negative_prompt: str, resolution: str, prompt_adherence: float, seed: int, steps: int) -> IO.NodeOutput:
        validate_string(prompt, min_length=1, max_length=MOONVALLEY_MAREY_MAX_PROMPT_LENGTH)
        validate_string(negative_prompt, field_name='negative_prompt', max_length=MOONVALLEY_MAREY_MAX_PROMPT_LENGTH)
        width_height = parse_width_height_from_res(resolution)
        inference_params = MoonvalleyTextToVideoInferenceParams(negative_prompt=negative_prompt, steps=steps, seed=seed, guidance_scale=prompt_adherence, num_frames=128, width=width_height['width'], height=width_height['height'])
        task_creation_response = await sync_op(cls, endpoint=ApiEndpoint(path=API_TXT2VIDEO_ENDPOINT, method='POST'), response_model=MoonvalleyPromptResponse, data=MoonvalleyTextToVideoRequest(prompt_text=prompt, inference_params=inference_params))
        validate_task_creation_response(task_creation_response)
        final_response = await get_response(cls, task_creation_response.id)
        return IO.NodeOutput(await download_url_to_video_output(final_response.output_url))

class MoonvalleyExtension_ISO(ComfyExtension):

    @override
    async def get_node_list(self) -> list[type[IO.ComfyNode]]:
        return [MoonvalleyImg2VideoNode_ISO, MoonvalleyTxt2VideoNode_ISO, MoonvalleyVideo2VideoNode_ISO]

async def comfy_entrypoint() -> MoonvalleyExtension_ISO:
    return MoonvalleyExtension_ISO()