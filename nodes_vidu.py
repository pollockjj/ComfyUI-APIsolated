...
from comfy_api_nodes.util import ApiEndpoint, download_url_to_video_output, get_number_of_images, poll_op, sync_op, upload_images_to_comfyapi, validate_image_aspect_ratio, validate_image_dimensions, validate_images_aspect_ratio_closeness
...
class ViduImageToVideoNode_ISO(IO.ComfyNode):
    ...
    @classmethod
    def define_schema(cls):
        return IO.Schema(node_id='ViduImageToVideoNode_ISO', display_name='Vidu Image To Video Generation_ISO', category='api node/video/Vidu', description='Generate video from image and optional prompt', inputs=[IO.Combo.Input('model', options=VideoModelName, default=VideoModelName.vidu_q1, tooltip='Model name'), IO.Image.Input('image', tooltip='An image to be used as the start frame of the generated video'), IO.String.Input('prompt', multiline=True, default='', tooltip='A textual description for video generation', optional=True), IO.Int.Input('duration', default=5, min=5, max=5, step=1, display_mode=IO.NumberDisplay.number, tooltip='Duration of the output video in seconds', optional=True), IO.Int.Input('seed', default=0, min=0, max=2147483647, step=1, display_mode=IO.NumberDisplay.number, control_after_generate=True, tooltip='Seed for video generation (0 for random)', optional=True), IO.Combo.Input('resolution', options=Resolution, default=Resolution.r_1080p, tooltip='Supported values may vary by model & duration', optional=True), IO.Combo.Input('movement_amplitude', options=MovementAmplitude, default=MovementAmplitude.auto.value, tooltip='The movement amplitude of objects in the frame', optional=True)], outputs=[IO.Video.Output()], hidden=[IO.Hidden.auth_token_comfy_org, IO.Hidden.api_key_comfy_org, IO.Hidden.unique_id], is_api_node=True)

    @classmethod
    async def execute(cls, model: str, image: torch.Tensor, prompt: str, duration: int, seed: int, resolution: str, movement_amplitude: str) -> IO.NodeOutput:
        if get_number_of_images(image) > 1:
            raise ValueError('Only one input image is allowed.')
        validate_image_aspect_ratio(image, (1, 4), (4, 1))
        payload = TaskCreationRequest(model_name=model, prompt=prompt, duration=duration, seed=seed, resolution=resolution, movement_amplitude=movement_amplitude)
        payload.images = await upload_images_to_comfyapi(cls, image, max_images=1, mime_type='image/png')
...
class ViduReferenceVideoNode_ISO(IO.ComfyNode):
    ...
    async def execute(cls, model: str, images: torch.Tensor, prompt: str, duration: int, seed: int, aspect_ratio: str, resolution: str, movement_amplitude: str) -> IO.NodeOutput:
        ...
        payload = TaskCreationRequest(model_name=model, prompt=prompt, duration=duration, seed=seed, aspect_ratio=aspect_ratio, resolution=resolution, movement_amplitude=movement_amplitude)
        payload.images = await upload_images_to_comfyapi(cls, images, max_images=7, mime_type='image/png')
...
class ViduStartEndToVideoNode_ISO(IO.ComfyNode):
    ...
    @classmethod
    def define_schema(cls):
        return IO.Schema(node_id='ViduStartEndToVideoNode_ISO', display_name='Vidu Start End To Video Generation_ISO', category='api node/video/Vidu', description='Generate a video from start and end frames and a prompt', inputs=[IO.Combo.Input('model', options=[model.value for model in VideoModelName], default=VideoModelName.vidu_q1.value, tooltip='Model name'), IO.Image.Input('first_frame', tooltip='Start frame'), IO.Image.Input('end_frame', tooltip='End frame'), IO.String.Input('prompt', multiline=True, tooltip='A textual description for video generation', optional=True), IO.Int.Input('duration', default=5, min=5, max=5, step=1, display_mode=IO.NumberDisplay.number, tooltip='Duration of the output video in seconds', optional=True), IO.Int.Input('seed', default=0, min=0, max=2147483647, step=1, display_mode=IO.NumberDisplay.number, control_after_generate=True, tooltip='Seed for video generation (0 for random)', optional=True), IO.Combo.Input('resolution', options=[model.value for model in Resolution], default=Resolution.r_1080p.value, tooltip='Supported values may vary by model & duration', optional=True), IO.Combo.Input('movement_amplitude', options=[model.value for model in MovementAmplitude], default=MovementAmplitude.auto.value, tooltip='The movement amplitude of objects in the frame', optional=True)], outputs=[IO.Video.Output()], hidden=[IO.Hidden.auth_token_comfy_org, IO.Hidden.api_key_comfy_org, IO.Hidden.unique_id], is_api_node=True)

    @classmethod
    async def execute(cls, model: str, first_frame: torch.Tensor, end_frame: torch.Tensor, prompt: str, duration: int, seed: int, resolution: str, movement_amplitude: str) -> IO.NodeOutput:
        validate_images_aspect_ratio_closeness(first_frame, end_frame, min_rel=0.8, max_rel=1.25, strict=False)
        payload = TaskCreationRequest(model_name=model, prompt=prompt, duration=duration, seed=seed, resolution=resolution, movement_amplitude=movement_amplitude)
        payload.images = [(await upload_images_to_comfyapi(cls, frame, max_images=1, mime_type='image/png'))[0] for frame in (first_frame, end_frame)]
        results = await execute_task(cls, VIDU_START_END_VIDEO, payload, 96)
        return IO.NodeOutput(await download_url_to_video_output(get_video_from_response(results).url))

class ViduExtension_ISO(ComfyExtension):

    @override
    async def get_node_list(self) -> list[type[IO.ComfyNode]]:
        return [ViduTextToVideoNode_ISO, ViduImageToVideoNode_ISO, ViduReferenceVideoNode_ISO, ViduStartEndToVideoNode_ISO]

async def comfy_entrypoint() -> ViduExtension_ISO:
    return ViduExtension_ISO()