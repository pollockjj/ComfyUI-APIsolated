...
async def execute_task(task_id: str, cls: type[IO.ComfyNode]) -> IO.NodeOutput:
    final_response: pika_defs.PikaVideoResponse = await poll_op(cls, ApiEndpoint(path=f'{PATH_VIDEO_GET}/{task_id}'), response_model=pika_defs.PikaVideoResponse, status_extractor=lambda response: response.status.value if response.status else None, progress_extractor=lambda response: response.progress if hasattr(response, 'progress') else None, estimated_duration=60, max_poll_attempts=240)
...
def get_base_inputs_types() -> list[IO.Input]:
    ...
    return [IO.String.Input('prompt_text', multiline=True), IO.String.Input('negative_prompt', multiline=True), IO.Int.Input('seed', min=0, max=4294967295, control_after_generate=True), IO.Combo.Input('resolution', options=['1080p', '720p'], default='1080p'), IO.Combo.Input('duration', options=[5, 10], default=5)]
...
class PikaImageToVideo_ISO(IO.ComfyNode):
    ...
    def define_schema(cls) -> IO.Schema:
        return IO.Schema(node_id='PikaImageToVideoNode2_2_ISO', display_name='Pika Image to Video_ISO', description='Sends an image and prompt to the Pika API v2.2 to generate a video.', category='api node/video/Pika', inputs=[IO.Image.Input('image', tooltip='The image to convert to video'), *get_base_inputs_types()], outputs=[IO.Video.Output()], hidden=[IO.Hidden.auth_token_comfy_org, IO.Hidden.api_key_comfy_org, IO.Hidden.unique_id], is_api_node=True)

    @classmethod
    async def execute(cls, image: torch.Tensor, prompt_text: str, negative_prompt: str, seed: int, resolution: str, duration: int) -> IO.NodeOutput:
        image_bytes_io = tensor_to_bytesio(image)
        pika_files = {'image': ('image.png', image_bytes_io, 'image/png')}
        pika_request_data = pika_defs.PikaBodyGenerate22I2vGenerate22I2vPost(promptText=prompt_text, negativePrompt=negative_prompt, seed=seed, resolution=resolution, duration=duration)
        initial_operation = await sync_op(cls, ApiEndpoint(path=PATH_IMAGE_TO_VIDEO, method='POST'), response_model=pika_defs.PikaGenerateResponse, data=pika_request_data, files=pika_files, content_type='multipart/form-data')
...
class PikaScenes_ISO(IO.ComfyNode):
    ...
    async def execute(cls, prompt_text: str, negative_prompt: str, seed: int, resolution: str, duration: int, ingredients_mode: str, aspect_ratio: float, image_ingredient_1: Optional[torch.Tensor]=None, image_ingredient_2: Optional[torch.Tensor]=None, image_ingredient_3: Optional[torch.Tensor]=None, image_ingredient_4: Optional[torch.Tensor]=None, image_ingredient_5: Optional[torch.Tensor]=None) -> IO.NodeOutput:
        all_image_bytes_io = []
        for image in [image_ingredient_1, image_ingredient_2, image_ingredient_3, image_ingredient_4, image_ingredient_5]:
            if image is not None:
                all_image_bytes_io.append(tensor_to_bytesio(image))
        pika_files = [('images', (f'image_{i}.png', image_bytes_io, 'image/png')) for i, image_bytes_io in enumerate(all_image_bytes_io)]
        pika_request_data = pika_defs.PikaBodyGenerate22C2vGenerate22PikascenesPost(ingredientsMode=ingredients_mode, promptText=prompt_text, negativePrompt=negative_prompt, seed=seed, resolution=resolution, duration=duration, aspectRatio=aspect_ratio)
        initial_operation = await sync_op(cls, ApiEndpoint(path=PATH_PIKASCENES, method='POST'), response_model=pika_defs.PikaGenerateResponse, data=pika_request_data, files=pika_files, content_type='multipart/form-data')
        return await execute_task(initial_operation.video_id, cls)
...
class PikAdditionsNode_ISO(IO.ComfyNode):
    ...
    @classmethod
    def define_schema(cls) -> IO.Schema:
        return IO.Schema(node_id='Pikadditions_ISO', display_name='Pikadditions (Video Object Insertion)_ISO', description="Add any object or image into your video. Upload a video and specify what you'd like to add to create a seamlessly integrated result.", category='api node/video/Pika', inputs=[IO.Video.Input('video', tooltip='The video to add an image to.'), IO.Image.Input('image', tooltip='The image to add to the video.'), IO.String.Input('prompt_text', multiline=True), IO.String.Input('negative_prompt', multiline=True), IO.Int.Input('seed', min=0, max=4294967295, control_after_generate=True)], outputs=[IO.Video.Output()], hidden=[IO.Hidden.auth_token_comfy_org, IO.Hidden.api_key_comfy_org, IO.Hidden.unique_id], is_api_node=True)

    @classmethod
    async def execute(cls, video: VideoInput, image: torch.Tensor, prompt_text: str, negative_prompt: str, seed: int) -> IO.NodeOutput:
        video_bytes_io = BytesIO()
        video.save_to(video_bytes_io, format=VideoContainer.MP4, codec=VideoCodec.H264)
        video_bytes_io.seek(0)
        image_bytes_io = tensor_to_bytesio(image)
        pika_files = {'video': ('video.mp4', video_bytes_io, 'video/mp4'), 'image': ('image.png', image_bytes_io, 'image/png')}
        pika_request_data = pika_defs.PikaBodyGeneratePikadditionsGeneratePikadditionsPost(promptText=prompt_text, negativePrompt=negative_prompt, seed=seed)
        initial_operation = await sync_op(cls, ApiEndpoint(path=PATH_PIKADDITIONS, method='POST'), response_model=pika_defs.PikaGenerateResponse, data=pika_request_data, files=pika_files, content_type='multipart/form-data')
        return await execute_task(initial_operation.video_id, cls)
...
class PikaSwapsNode_ISO(IO.ComfyNode):
    ...
    async def execute(cls, video: VideoInput, image: Optional[torch.Tensor]=None, mask: Optional[torch.Tensor]=None, prompt_text: str='', negative_prompt: str='', seed: int=0, region_to_modify: str='') -> IO.NodeOutput:
        ...
        pika_request_data = pika_defs.PikaBodyGeneratePikaswapsGeneratePikaswapsPost(promptText=prompt_text, negativePrompt=negative_prompt, seed=seed, modifyRegionRoi=region_to_modify if region_to_modify else None)
        initial_operation = await sync_op(cls, ApiEndpoint(path=PATH_PIKASWAPS, method='POST'), response_model=pika_defs.PikaGenerateResponse, data=pika_request_data, files=pika_files, content_type='multipart/form-data')
        return await execute_task(initial_operation.video_id, cls)
...
class PikaStartEndFrameNode_ISO(IO.ComfyNode):
    ...
    @classmethod
    def define_schema(cls) -> IO.Schema:
        return IO.Schema(node_id='PikaStartEndFrameNode2_2_ISO', display_name='Pika Start and End Frame to Video_ISO', description='Generate a video by combining your first and last frame. Upload two images to define the start and end points, and let the AI create a smooth transition between them.', category='api node/video/Pika', inputs=[IO.Image.Input('image_start', tooltip='The first image to combine.'), IO.Image.Input('image_end', tooltip='The last image to combine.'), *get_base_inputs_types()], outputs=[IO.Video.Output()], hidden=[IO.Hidden.auth_token_comfy_org, IO.Hidden.api_key_comfy_org, IO.Hidden.unique_id], is_api_node=True)

    @classmethod
    async def execute(cls, image_start: torch.Tensor, image_end: torch.Tensor, prompt_text: str, negative_prompt: str, seed: int, resolution: str, duration: int) -> IO.NodeOutput:
        validate_string(prompt_text, field_name='prompt_text', min_length=1)
        pika_files = [('keyFrames', ('image_start.png', tensor_to_bytesio(image_start), 'image/png')), ('keyFrames', ('image_end.png', tensor_to_bytesio(image_end), 'image/png'))]
        initial_operation = await sync_op(cls, ApiEndpoint(path=PATH_PIKAFRAMES, method='POST'), response_model=pika_defs.PikaGenerateResponse, data=pika_defs.PikaBodyGenerate22KeyframeGenerate22PikaframesPost(promptText=prompt_text, negativePrompt=negative_prompt, seed=seed, resolution=resolution, duration=duration), files=pika_files, content_type='multipart/form-data')
        return await execute_task(initial_operation.video_id, cls)

class PikaApiNodesExtension_ISO(ComfyExtension):

    @override
    async def get_node_list(self) -> list[type[IO.ComfyNode]]:
        return [PikaImageToVideo_ISO, PikaTextToVideoNode_ISO, PikaScenes_ISO, PikAdditionsNode_ISO, PikaSwapsNode_ISO, PikaffectsNode_ISO, PikaStartEndFrameNode_ISO]

async def comfy_entrypoint() -> PikaApiNodesExtension_ISO:
    return PikaApiNodesExtension_ISO()