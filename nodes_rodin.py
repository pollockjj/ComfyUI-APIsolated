...
async def poll_for_task_status(subscription_key: str, cls: type[IO.ComfyNode]) -> Rodin3DCheckStatusResponse:
    ...
    return await poll_op(cls, ApiEndpoint(path='/proxy/rodin/api/v2/status', method='POST'), response_model=Rodin3DCheckStatusResponse, data=Rodin3DCheckStatusRequest(subscription_key=subscription_key), status_extractor=check_rodin_status, progress_extractor=extract_progress)
...
async def get_rodin_download_list(uuid: str, cls: type[IO.ComfyNode]) -> Rodin3DDownloadResponse:
    ...
    return await sync_op(cls, ApiEndpoint(path='/proxy/rodin/api/v2/download', method='POST'), response_model=Rodin3DDownloadResponse, data=Rodin3DDownloadRequest(task_uuid=uuid), monitor_progress=False)
...
class Rodin3D_Regular_ISO(IO.ComfyNode):
    ...
    def define_schema(cls) -> IO.Schema:
        return IO.Schema(node_id='Rodin3D_Regular_ISO', display_name='Rodin 3D Generate - Regular Generate_ISO', category='api node/3d/Rodin', description=cleandoc(cls.__doc__ or ''), inputs=[IO.Image.Input('Images'), *COMMON_PARAMETERS], outputs=[IO.String.Output(display_name='3D Model Path')], hidden=[IO.Hidden.auth_token_comfy_org, IO.Hidden.api_key_comfy_org, IO.Hidden.unique_id], is_api_node=True)

    @classmethod
    async def execute(cls, Images, Seed, Material_Type, Polygon_count) -> IO.NodeOutput:
        tier = 'Regular'
        ...
        task_uuid, subscription_key = await create_generate_task(cls, images=m_images, seed=Seed, material=Material_Type, quality_override=quality_override, tier=tier, mesh_mode=mesh_mode)
...
class Rodin3D_Detail_ISO(IO.ComfyNode):
    ...
    @classmethod
    def define_schema(cls) -> IO.Schema:
        return IO.Schema(node_id='Rodin3D_Detail_ISO', display_name='Rodin 3D Generate - Detail Generate_ISO', category='api node/3d/Rodin', description=cleandoc(cls.__doc__ or ''), inputs=[IO.Image.Input('Images'), *COMMON_PARAMETERS], outputs=[IO.String.Output(display_name='3D Model Path')], hidden=[IO.Hidden.auth_token_comfy_org, IO.Hidden.api_key_comfy_org, IO.Hidden.unique_id], is_api_node=True)
...
class Rodin3D_Smooth_ISO(IO.ComfyNode):
    ...
    @classmethod
    def define_schema(cls) -> IO.Schema:
        return IO.Schema(node_id='Rodin3D_Smooth_ISO', display_name='Rodin 3D Generate - Smooth Generate_ISO', category='api node/3d/Rodin', description=cleandoc(cls.__doc__ or ''), inputs=[IO.Image.Input('Images'), *COMMON_PARAMETERS], outputs=[IO.String.Output(display_name='3D Model Path')], hidden=[IO.Hidden.auth_token_comfy_org, IO.Hidden.api_key_comfy_org, IO.Hidden.unique_id], is_api_node=True)
    ...
    async def execute(cls, Images, Seed, Material_Type, Polygon_count) -> IO.NodeOutput:
        ...
        for i in range(num_images):
            m_images.append(Images[i])
        mesh_mode, quality_override = get_quality_mode(Polygon_count)
        task_uuid, subscription_key = await create_generate_task(cls, images=m_images, seed=Seed, material=Material_Type, quality_override=quality_override, tier='Smooth', mesh_mode=mesh_mode)
        await poll_for_task_status(subscription_key, cls)
        download_list = await get_rodin_download_list(task_uuid, cls)
        model = await download_files(download_list, task_uuid)
        return IO.NodeOutput(model)
...
class Rodin3D_Sketch_ISO(IO.ComfyNode):
    ...
    @classmethod
    def define_schema(cls) -> IO.Schema:
        return IO.Schema(node_id='Rodin3D_Sketch_ISO', display_name='Rodin 3D Generate - Sketch Generate_ISO', category='api node/3d/Rodin', description=cleandoc(cls.__doc__ or ''), inputs=[IO.Image.Input('Images'), IO.Int.Input('Seed', default=0, min=0, max=65535, display_mode=IO.NumberDisplay.number, optional=True)], outputs=[IO.String.Output(display_name='3D Model Path')], hidden=[IO.Hidden.auth_token_comfy_org, IO.Hidden.api_key_comfy_org, IO.Hidden.unique_id], is_api_node=True)

    @classmethod
    async def execute(cls, Images, Seed) -> IO.NodeOutput:
        num_images = Images.shape[0]
        m_images = []
        for i in range(num_images):
            m_images.append(Images[i])
        task_uuid, subscription_key = await create_generate_task(cls, images=m_images, seed=Seed, material='PBR', quality_override=18000, tier='Sketch', mesh_mode='Quad')
        await poll_for_task_status(subscription_key, cls)
        download_list = await get_rodin_download_list(task_uuid, cls)
        model = await download_files(download_list, task_uuid)
        return IO.NodeOutput(model)
...
class Rodin3D_Gen2_ISO(IO.ComfyNode):
    ...
    async def execute(cls, Images, Seed, Material_Type, Polygon_count, TAPose) -> IO.NodeOutput:
        ...
        for i in range(num_images):
            m_images.append(Images[i])
        mesh_mode, quality_override = get_quality_mode(Polygon_count)
        task_uuid, subscription_key = await create_generate_task(cls, images=m_images, seed=Seed, material=Material_Type, quality_override=quality_override, tier=tier, mesh_mode=mesh_mode, ta_pose=TAPose)
        await poll_for_task_status(subscription_key, cls)
        download_list = await get_rodin_download_list(task_uuid, cls)
        model = await download_files(download_list, task_uuid)
        return IO.NodeOutput(model)

class Rodin3DExtension_ISO(ComfyExtension):

    @override
    async def get_node_list(self) -> list[type[IO.ComfyNode]]:
        return [Rodin3D_Regular_ISO, Rodin3D_Detail_ISO, Rodin3D_Smooth_ISO, Rodin3D_Sketch_ISO, Rodin3D_Gen2_ISO]

async def comfy_entrypoint() -> Rodin3DExtension_ISO:
    return Rodin3DExtension_ISO()