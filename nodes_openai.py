...
from comfy_api_nodes.apis import OpenAIImageGenerationRequest, OpenAIImageEditRequest, OpenAIImageGenerationResponse, OpenAICreateResponse, OpenAIResponse, CreateModelResponseProperties, Item, OutputContent, InputImageContent, Detail, InputTextContent, InputMessage, InputMessageContentList, InputContent, InputFileContent
from comfy_api_nodes.util import downscale_image_tensor, download_url_to_bytesio, validate_string, tensor_to_base64_string, ApiEndpoint, sync_op, poll_op, text_filepath_to_data_uri
RESPONSES_ENDPOINT = '/proxy/openai/v1/responses'
STARTING_POINT_ID_PATTERN = '<starting_point_id:(.*)>'
...
class OpenAIDalle2_ISO(IO.ComfyNode):
    ...
    async def execute(cls, prompt, seed=0, image=None, mask=None, n=1, size='1024x1024') -> IO.NodeOutput:
        ...
        response = await sync_op(cls, ApiEndpoint(path=path, method='POST'), response_model=OpenAIImageGenerationResponse, data=request_class(model=model, prompt=prompt, n=n, size=size, seed=seed), files={'image': ('image.png', img_binary, 'image/png')} if img_binary else None, content_type=content_type)
...
class OpenAIDalle3_ISO(IO.ComfyNode):
    ...
    @classmethod
    def define_schema(cls):
        return IO.Schema(node_id='OpenAIDalle3_ISO', display_name='OpenAI DALL·E 3_ISO', category='api node/image/OpenAI', description=cleandoc(cls.__doc__ or ''), inputs=[IO.String.Input('prompt', default='', multiline=True, tooltip='Text prompt for DALL·E'), IO.Int.Input('seed', default=0, min=0, max=2 ** 31 - 1, step=1, display_mode=IO.NumberDisplay.number, control_after_generate=True, tooltip='not implemented yet in backend', optional=True), IO.Combo.Input('quality', default='standard', options=['standard', 'hd'], tooltip='Image quality', optional=True), IO.Combo.Input('style', default='natural', options=['natural', 'vivid'], tooltip='Vivid causes the model to lean towards generating hyper-real and dramatic images. Natural causes the model to produce more natural, less hyper-real looking images.', optional=True), IO.Combo.Input('size', default='1024x1024', options=['1024x1024', '1024x1792', '1792x1024'], tooltip='Image size', optional=True)], outputs=[IO.Image.Output()], hidden=[IO.Hidden.auth_token_comfy_org, IO.Hidden.api_key_comfy_org, IO.Hidden.unique_id], is_api_node=True)

    @classmethod
    async def execute(cls, prompt, seed=0, style='natural', quality='standard', size='1024x1024') -> IO.NodeOutput:
        validate_string(prompt, strip_whitespace=False)
        model = 'dall-e-3'
        response = await sync_op(cls, ApiEndpoint(path='/proxy/openai/images/generations', method='POST'), response_model=OpenAIImageGenerationResponse, data=OpenAIImageGenerationRequest(model=model, prompt=prompt, quality=quality, size=size, style=style, seed=seed))
        return IO.NodeOutput(await validate_and_cast_response(response))
...
class OpenAIGPTImage1_ISO(IO.ComfyNode):
    ...
    async def execute(cls, prompt, seed=0, quality='low', background='opaque', image=None, mask=None, n=1, size='1024x1024') -> IO.NodeOutput:
        ...
        response = await sync_op(cls, ApiEndpoint(path=path, method='POST'), response_model=OpenAIImageGenerationResponse, data=request_class(model=model, prompt=prompt, quality=quality, background=background, n=n, seed=seed, size=size), files=files if files else None, content_type=content_type)
        return IO.NodeOutput(await validate_and_cast_response(response))
...
class OpenAIChatNode_ISO(IO.ComfyNode):
    ...
    @classmethod
    def tensor_to_input_image_content(cls, image: torch.Tensor, detail_level: Detail='auto') -> InputImageContent:
        """Convert a tensor to an input image content object."""
        return InputImageContent(detail=detail_level, image_url=f'data:image/png;base64,{tensor_to_base64_string(image)}', type='input_image')

    @classmethod
    def create_input_message_contents(cls, prompt: str, image: torch.Tensor | None=None, files: list[InputFileContent] | None=None) -> InputMessageContentList:
        """Create a list of input message contents from prompt and optional image."""
        content_list: list[InputContent | InputTextContent | InputImageContent | InputFileContent] = [InputTextContent(text=prompt, type='input_text')]
        if image is not None:
            for i in range(image.shape[0]):
                content_list.append(InputImageContent(detail='auto', image_url=f'data:image/png;base64,{tensor_to_base64_string(image[i].unsqueeze(0))}', type='input_image'))
        if files is not None:
            content_list.extend(files)
        return InputMessageContentList(root=content_list)

    @classmethod
    async def execute(cls, prompt: str, persist_context: bool=False, model: SupportedOpenAIModel=SupportedOpenAIModel.gpt_5.value, images: torch.Tensor | None=None, files: list[InputFileContent] | None=None, advanced_options: CreateModelResponseProperties | None=None) -> IO.NodeOutput:
        validate_string(prompt, strip_whitespace=False)
        create_response = await sync_op(cls, ApiEndpoint(path=RESPONSES_ENDPOINT, method='POST'), response_model=OpenAIResponse, data=OpenAICreateResponse(input=[Item(root=InputMessage(content=cls.create_input_message_contents(prompt, images, files), role='user'))], store=True, stream=False, model=model, previous_response_id=None, **advanced_options.model_dump(exclude_none=True) if advanced_options else {}))
        response_id = create_response.id
        result_response = await poll_op(cls, ApiEndpoint(path=f'{RESPONSES_ENDPOINT}/{response_id}'), response_model=OpenAIResponse, status_extractor=lambda response: response.status, completed_statuses=['incomplete', 'completed'])
...
class OpenAIChatConfig_ISO(IO.ComfyNode):
    ...
    @classmethod
    def define_schema(cls):
        return IO.Schema(node_id='OpenAIChatConfig_ISO', display_name='OpenAI ChatGPT Advanced Options_ISO', category='api node/text/OpenAI', description='Allows specifying advanced configuration options for the OpenAI Chat Nodes.', inputs=[IO.Combo.Input('truncation', options=['auto', 'disabled'], default='auto', tooltip="The truncation strategy to use for the model response. auto: If the context of this response and previous ones exceeds the model's context window size, the model will truncate the response to fit the context window by dropping input items in the middle of the conversation.disabled: If a model response will exceed the context window size for a model, the request will fail with a 400 error"), IO.Int.Input('max_output_tokens', min=16, default=4096, max=16384, tooltip='An upper bound for the number of tokens that can be generated for a response, including visible output tokens', optional=True), IO.String.Input('instructions', multiline=True, optional=True, tooltip='Instructions for the model on how to generate the response')], outputs=[IO.Custom('OPENAI_CHAT_CONFIG').Output()])

    @classmethod
    def execute(cls, truncation: bool, instructions: str | None=None, max_output_tokens: int | None=None) -> IO.NodeOutput:
        """
        ...
        """
        return IO.NodeOutput(CreateModelResponseProperties(instructions=instructions, truncation=truncation, max_output_tokens=max_output_tokens))

class OpenAIExtension_ISO(ComfyExtension):

    @override
    async def get_node_list(self) -> list[type[IO.ComfyNode]]:
        return [OpenAIDalle2_ISO, OpenAIDalle3_ISO, OpenAIGPTImage1_ISO, OpenAIChatNode_ISO, OpenAIInputFiles_ISO, OpenAIChatConfig_ISO]

async def comfy_entrypoint() -> OpenAIExtension_ISO:
    return OpenAIExtension_ISO()